...
- **`AsaasClient`** (`src/AsaasClient.php`) — entry point. Lazy-resolves one Resource per API domain (`payments()`, `leanPayments()`, `pix()`, `pixTransactions()`, `pixAutomatic()`, `transfers()`, `webhooks()`, `invoices()`, `accounts()`, `myAccount()`, `creditCards()`, `billPayments()`, `statements()`, `fiscalInfo()`). Constructed via `AsaasClient::for()` for standalone usage, or via the Laravel container/`Asaas` facade.
